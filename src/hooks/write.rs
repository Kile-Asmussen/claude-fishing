...
pub fn check(project_dir: &Path, env: &mut HookEnv, tool: &str, file_path: &Path) {
    ...
    let rel = match file_path.strip_prefix(project_dir) {
        Ok(r) => r,
        Err(_) => return env.deny("could not relativise path".to_string()),
    };
    ...
    env.deny(format!(
        ...
        globs.iter().map(|g| format!("{tool}({g})")).collect::<Vec<_>>().join(", ")
    ))
}
...
mod tests {
    ...
    fn settings(json: &str) -> HookEnv { HookEnv::test("", "", "", json) }
    ...
    fn allows_matching_path() {
        ...
        super::check(Path::new("/proj"), &mut env, "Edit", Path::new("/proj/src/main.rs"));
    ...
    fn denies_unmatched_path() {
        let mut env = settings(SETTINGS_WITH_EDIT);
        super::check(Path::new("/proj"), &mut env, "Edit", Path::new("/proj/other/file.txt"));
    ...
    fn denies_outside_project() {
        let mut env = settings(SETTINGS_WITH_EDIT);
        super::check(Path::new("/proj"), &mut env, "Edit", Path::new("/etc/passwd"));
        assert_eq!(env.decision(), Some(&PreToolDecision::Deny));
    }
    ...
    fn denies_inside_claude_dir() {
        let mut env = settings(SETTINGS_WITH_EDIT);
        super::check(Path::new("/proj"), &mut env, "Edit", Path::new("/proj/.claude/settings.json"));
        assert_eq!(env.decision(), Some(&PreToolDecision::Deny));
    }
    ...
    fn tool_patterns_are_not_shared_across_tools() {
        ...
        super::check(Path::new("/proj"), &mut env, "Write", Path::new("/proj/src/main.rs"));
        assert_eq!(env.decision(), Some(&PreToolDecision::Deny));
    }
}
...
fn load_globs(env: &mut HookEnv, tool: &str) -> Result<Vec<String>, String> {
    ...
    Ok(allow.into_iter().filter_map(|entry| {
        entry.strip_prefix(&prefix)?.strip_suffix(')')?.to_string().into()
    }).collect())
}
